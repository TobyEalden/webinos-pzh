...
// APPG
        ...
        "addTransactionResponse": addTransactionResponse,
  ...
  function pzhNicknameToUserId(nickname, host, port) {
    ...
    function processMsg (conn,message) {
        ...
        if (validateMessage (message)) {
            if( message.message === "WEB SERVER INIT") {
                ...
                if (userObj === null) {
                  userObj = findExistingUserFromEmail(message.user.nickname);
                }
                if (userObj) {
    ...
    // APPG
    function getNotificationConfig(conn, obj, userObj) {
      var cfg = userObj.notificationManager.getConfig();
      sendMsg (conn, obj.user, { type:"getNotificationConfig", message:cfg });
    }

    function addNotification(conn, obj, userObj) {
      var n = userObj.notificationManager.addNotification(obj.message.notificationType, obj.message.data);
      sendMsg (conn, obj.user, { type:"addNotification", message: n });
    }

    function addTransactionResponse(conn, obj, userObj) {
      // Need to find transaction notification.
      var transNotifications = userObj.notificationManager.getNotifications(userObj.notificationManager.notifyType.appgTransaction);
      var transNotify;
      for (var n in transNotifications.notifications) {
        if (transNotifications.notifications[n].data.id === obj.message.data.responseToTransaction) {
          transNotify = transNotifications.notifications[n];
          break;
        }
      }
      if (typeof transNotify !== "undefined") {
        var n = userObj.notificationManager.addNotification(userObj.notificationManager.notifyType.appgTransactionResponse, { responseTo: transNotify.id, response: obj.message.data.response});
        sendMsg (conn, obj.user, { type:"addTransactionResponse", message: n });
      }
    }

    function saveNotificationConfig(conn, obj, userObj) {
      var key = obj.message.key;
      var subKey = obj.message.subKey;
      var value = obj.message.value;
      var notificationConfig = userObj.notificationManager.getConfig();
      notificationConfig[key][subKey] = value;
      userObj.notificationManager.setConfig(notificationConfig);
      userObj.synchronizationStartDelayed();
      sendMsg (conn, obj.user, { type:"saveNotificationConfig", message: { ok: true }});
    }
};
